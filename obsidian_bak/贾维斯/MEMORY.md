...
## 视频分析 pipeline 规则（2026-04-12）

**收到视频 URL 时**，通过 sub-agent 处理，不占用主 agent 会话：
- 主 agent spawn isolated sub-agent（`mode=run`）
- Sub-agent 调用 bibi CLI 分析视频
- 分析完成后**纯本地保存到 Obsidian**（`~/Library/Mobile Documents/iCloud~md~obsidian/Documents/docs/BibiGPT/`）
- **不通知用户**（不主动发消息）

**bibi 认证方式**：桌面端 session token（`bibi auth check` 显示 `Auth: desktop session`），不需要 `BIBI_API_TOKEN`。

### 网站查总结规则（2026-04-12）

**收到网站 URL 时**，通过 sub-agent 处理，不占用主 agent 会话：
- 主 agent spawn isolated sub-agent（`mode=run`）
- Sub-agent 用 `web_fetch` 或 `browser` 工具抓取页面内容
- 总结页面要点，保存到 Obsidian（`~/Library/Mobile Documents/iCloud~md~obsidian/Documents/docs/WebNotes/`）
- **不通知用户**（不主动发消息）

**触发词**："查一下这个网站"、"帮我看看这个页面"、"总结一下" + URL 等。

### URL 路由规则（2026-04-12）

| URL 类型 | 处理方式 |
|---------|---------|
| 视频平台（YouTube/B站/抖音/TikTok/小宇宙等） | bibi sub-agent |
| 其他所有 URL | 网页总结 sub-agent |

规则如有例外或问题，后续更新。

### 编程 sub-agent 规则（2026-04-12）

**收到编程请求时**，通过 sub-agent + coding-agent skill 处理：
- 主 agent 解析意图（写代码/查问题/代码审查）
- spawn sub-agent → 调用 coding-agent skill（Claude Code）
- 完成后主 agent 通知用户

| 任务类型 | 处理方式 |
|---------|---------|
| 写代码/修改代码 | spawn coding sub-agent → Claude Code 执行 → 完成后主 agent 通知 |
| 查问题/调试 | spawn coding sub-agent → Claude Code 分析 → 完成后主 agent 通知 |
| 代码审查/学习 | spawn sub-agent → 分析保存 Obsidian → 不通知 |
| 运维操作（部署/同步/重启等） | spawn sub-agent → 操作 + **自动验证**（检查服务是否恢复）→ 完成后主 agent 通知 |
